...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const emojiWrap = document.getElementById('emoji-picker-wrapper');
  let emojiPicker;
  ...
  function autosizeTextarea(el){ if(!el) return; el.style.height='auto'; var h=Math.min(el.scrollHeight,200); el.style.height=h+'px'; }
	elements.messageInput.addEventListener('input', function(){ autosizeTextarea(elements.messageInput); });
	autosizeTextarea(elements.messageInput);

	if (emojiBtn && emojiWrap && emojiPicker) {
	  emojiBtn.addEventListener('click', function(e){
		e.preventDefault();
		emojiWrap.classList.toggle('d-none');
	  });
	  document.addEventListener('click', function(e){
		if (!emojiWrap.contains(e.target) && !emojiBtn.contains(e.target)) emojiWrap.classList.add('d-none');
	  });
  ...
  function handleMaxLengthCount(input, info, max) { const len = input.value.length; info.className = len < max ? 'maxLength label-success' : 'maxLength label-danger'; info.textContent = `${len}/${max}`; }
  ...
  function appendToLi(li, m) {
    ...
    if (m.type === 'image' && m.attachment) {
      ...
      imgDiv.innerHTML = `<img src="${m.attachment}" style="max-width:140px;">`;
      wrapper.insertBefore(imgDiv, wrapper.querySelector('.mt-1'));
      ...
      const imgDiv = document.createElement('div');
      imgDiv.className = 'chat-message-text mt-2';
      imgDiv.innerHTML = `<audio style="max-width:250px;border-radius:8px;" controls><source src="${m.attachment}" type="audio/ogg"></audio>`;
      wrapper.insertBefore(imgDiv, wrapper.querySelector('.mt-1'));
    } else if (m.type === 'video' && m.attachment) {
      const imgDiv = document.createElement('div');
      imgDiv.className = 'chat-message-text mt-2';
      imgDiv.innerHTML = `<video style="width:100%;max-width:250px;border-radius:8px;" controls><source src="${m.attachment}" type="video/mp4"></video>`;
      wrapper.insertBefore(imgDiv, wrapper.querySelector('.mt-1'));
    } else if (m.type === 'document' && m.attachment) {
      const imgDiv = document.createElement('div');
      imgDiv.className = 'chat-message-text mt-2';
      imgDiv.innerHTML = `<a href="${m.attachment}" download><i class="icon-base ti tabler-file-type-doc icon-22px" style="max-width:250px;border-radius:8px;"></i> ${m.original_file || ''}</a>`;
      wrapper.insertBefore(imgDiv, wrapper.querySelector('.mt-1'));
    } else {
  ...
  function buildMessageGroupHtml(m, includeAvatar) {
    ...
    let contentHtml;
    if (m.type === 'image' && m.attachment) {
      contentHtml = `<div class="chat-message-wrapper flex-grow-1"><div class="chat-message-text"><img src="${m.attachment}" style="max-width:250px;border-radius:8px;cursor:pointer;" data-bs-toggle="modal" data-bs-target="#imageModal" data-bs-src="${m.attachment}" /></div><div class="${isIncoming ? 'text-body-secondary' : 'text-end'} mt-1"><small>${new Date(m.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</small></div></div>`;
    } else if (m.type === 'sticker' && m.attachment) {
      contentHtml = `<div class="chat-message-wrapper flex-grow-1"><img src="${m.attachment}" style="max-width:140px;"><div class="${isIncoming ? 'text-body-secondary' : 'text-end'} mt-1"><small>${new Date(m.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</small></div></div>`;
    } else if (m.type === 'location') {
      const { degreesLatitude: lat, degreesLongitude: lng } = JSON.parse(m.attachment);
      const mapUrl = `https://www.google.com/maps?q=${lat},${lng}`;
      const embedSrc = `https://maps.google.com/maps?q=${lat},${lng}&z=15&output=embed`;
      contentHtml = `<div class="chat-message-wrapper flex-grow-1"><div class="chat-message-text"><a href="${mapUrl}" target="_blank"><iframe src="${embedSrc}" width="200" height="150" style="border:0;border-radius:8px;"></iframe></a></div><div class="${isIncoming ? 'text-body-secondary' : 'text-end'} mt-1"><small>${new Date(m.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</small></div></div>`;
    } else if (m.type === 'audio' && m.attachment) {
      contentHtml = `<div class="chat-message-wrapper flex-grow-1"><div class="chat-message-text"><audio style="max-width:250px;border-radius:8px;" controls><source src="${m.attachment}" type="audio/ogg"></audio></div><div class="${isIncoming ? 'text-body-secondary' : 'text-end'} mt-1"><small>${new Date(m.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</small></div></div>`;
    } else if (m.type === 'video' && m.attachment) {
      contentHtml = `<div class="chat-message-wrapper flex-grow-1"><div class="chat-message-text"><video style="max-width:250px;border-radius:8px;" controls><source src="${m.attachment}" type="video/mp4"></video></div><div class="${isIncoming ? 'text-body-secondary' : 'text-end'} mt-1"><small>${new Date(m.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</small></div></div>`;
    } else if (m.type === 'vcard') {
      const vraw = decodeURIComponent(m.message)
      const lines = vraw.split('\n')
      const fnLine = (lines.find(l => l.startsWith('FN:')) || '').replace('FN:', '')
      ...
      contentHtml = `<div class="chat-message-wrapper flex-grow-1"><div class="chat-message-text"><p class="mb-0" style="white-space: break-spaces;">${formatWppMarkdown(m.message)}</p></div><div class="${isIncoming ? 'text-body-secondary' : 'text-end'} mt-1">${!isIncoming ? '<i class="icon-base ti tabler-checks icon-16px text-success me-1"></i>' : ''}<small>${new Date(m.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</small></div></div>`;
    }
    return `<div class="d-flex overflow-hidden">${isIncoming ? avatarBlock : ''}${contentHtml}${!isIncoming ? avatarBlock : ''}</div>`;
  }
  ...
  headerAvatarEl.addEventListener('click', e => {
	  requestContactInfo(e);
  });
